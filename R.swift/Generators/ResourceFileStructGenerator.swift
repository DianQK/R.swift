...
struct ResourceFileStructGenerator: ExternalOnlyStructGenerator {
  ...
  func generatedStruct(at externalAccessLevel: AccessLevel, withStructName structName: String) -> Struct {
    ...
    return Struct(
      comments: ["This `\(structName).file` struct is generated, and contains static references to \(firstLocales.count) files."],
      accessModifier: externalAccessLevel,
      type: Type(module: .host, name: "file"),
      implements: [],
      typealiasses: [],
      properties: firstLocales.flatMap { propertiesFromResourceFiles(resourceFiles: $0.1, at: externalAccessLevel, withStructName: structName) },
      functions: firstLocales.flatMap { functionsFromResourceFiles(resourceFiles: $0.1, at: externalAccessLevel, withStructName: structName) },
      structs: [],
      classes: []
    )
  }

  private func propertiesFromResourceFiles(resourceFiles: [ResourceFile], at externalAccessLevel: AccessLevel, withStructName structName: String) -> [Let] {
    ...
    return resourceFiles
      .map {
        return Let(
          ...
          value: "Rswift.FileResource(bundle: \(structName).hostingBundle, name: \"\($0.filename)\", pathExtension: \"\($0.pathExtension)\")"
        )
    }
  }

  private func functionsFromResourceFiles(resourceFiles: [ResourceFile], at externalAccessLevel: AccessLevel, withStructName structName: String) -> [Function] {
      ...
      .flatMap { resourceFile -> [Function] in
        ...
        return [
          Function(
            ...
            body: "let fileResource = \(structName).file.\(SwiftIdentifier(name: fullname))\nreturn fileResource.bundle.url(forResource: fileResource)"
          )
        ]
    }
  }
